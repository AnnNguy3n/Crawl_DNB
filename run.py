...
def crawl_all_df_company_href(list_state, num_proxy, num_thread, max_trial, start_index, last_index):
    for state in list_state:
        os.makedirs(f"{FOLDER_DATA}/{state}/URLs", exist_ok=True)
        print(f"{FOLDER_DATA}/{state}/URLs")
        crl = Crawler()
        crl.multithread_get_all_df_company_href(state, num_proxy, num_thread, max_trial, start_index, last_index)

# zip_proxy_extensions()
# crawl_df_industry_href(0)
# crawl_all_df_city_href(["florida", "texas"],
#                        0,
#                        10,
#                        1)
